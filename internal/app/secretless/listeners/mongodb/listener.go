...
import (
	...
	"github.com/cyberark/secretless-broker/internal/app/secretless/handlers/mongodb/protocol"
...
func (l *Listener) Listen() {
	for {
		...
		if len(l.HandlerConfigs) > 0 {
			...
			handler := l.RunHandlerFunc("mongodb", handlerOptions)
			l.AddHandler(handler)
		} else {
			mongoError := protocol.Error{
			...
			client.Write(mongoError.GetMessage())
		}
	}
}
