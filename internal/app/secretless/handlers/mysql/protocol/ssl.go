...
// Common error types
var (
	ErrSSLKeyHasWorldPermissions = errors.New("handler: Private key file has group or world access. Permissions should be u=rw (0600) or less")
)
...
func ResolveTLSConfig(o options) (TLSConfigWrapper, error) {
	...
	default:
		return TLSConfigWrapper{}, fmt.Errorf(`unsupported sslmode %q; only "require" (default), "verify-full", "verify-ca", and "disable" supported`, mode)
	}
